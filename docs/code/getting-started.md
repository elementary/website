...
Now it is time to incorporate the Unity library into your project.
  ...
  * If you use CMake as your build system, add an additional `unity` package to your `vala_precompile` CMake call:
  ...
  * If you use Meson, compiling with `libunity` is as simple as adding `unity` dependency to the `executable` call:
  ...
  Rebuild your project and you should good be go.
...
This is usually the filename of your application entry that is installed in `/usr/share/applications`
...
entry.count = 12;
```
Note that, you have to set the `count_visible` property to `true` when showing a badge counter. 
Notice the `int64` type of the `count` property. You cannot show real numbers in the badge itself.
...
Static quicklists are stored within your `.desktop` file. Those are so called "actions".
