...
$is_HTTP = (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] !== 'on');
$is_old_IE = preg_match('/MSIE [2-10]/i', $user_agent);

/**
 * Considering that modern versions of midori, like the one in Loki, work fine
 * with HTTPS we should only match freya versions or older. All versions of
 * midori include the 'Midori/0.5' user agent, so we have to assume it will mask
 * as an old chrome browser.
 */
$is_old_midori = preg_match('/CHROME\/18.0.1025.133/i', $user_agent);
...
<?php } else if ($is_old_midori) { ?>

    <div id="legacy-warning">
        <h1>The elementary OS website is built on modern web technologies your version of midori doesn&rsquo;t support.</h1>
        <p>This version of midori is out of date and does not support secure connections. Please <a href="http://browsehappy.com/">upgrade</a> midori or use an alternative web browser.</p>
        <div id="legacy-warning-buttons">
            <a href="#" onClick="document.getElementById('legacy-warning').style.display = 'none';">Dismiss</a>
            <a class="suggested-action" href="http://browsehappy.com/" target="_blank">Learn More</a>
        </div>
